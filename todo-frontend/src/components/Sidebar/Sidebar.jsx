...
const Sidebar = memo(({ todos, categories, isDarkMode, setIsDarkMode, onClear, onSelectCategory, activeFilter }) => {
  ...
  const stats = useMemo(() => {
    return todos.reduce((acc, todo) => {
      ...
      return acc;
    }, { completed: 0, active: 0, deleted: 0, total: 0, recovered: 0, catCounts: {} });
  }, [todos]);
          ...
          <SidebarDropdown title="WORKSPACE">
            <ul className="dropdown-list">
              <li 
                ...
                className={`category-item all-item ${activeFilter === 'All' ? 'active' : ''}`}
              >
              ...
              <li 
                onClick={() => handleCategoryClick('Recovered')} 
                className={`category-item recovered-item ${activeFilter === 'Recovered' ? 'active' : ''}`}
              >
                <div className="item-label">
                  <span className="icon">🟢</span>
                  <span>Recovered</span>
              ...
              <li 
                ...
                className={`category-item trash-item ${activeFilter === 'Trash' ? 'active' : ''}`}
              >
                <div className="item-label">
            ...
            <ul className="dropdown-list">
              {categories.map(catName => (
                <li 
                  ...
                  className={`category-item ${catName.toLowerCase()}-item ${activeFilter === catName ? 'active' : ''}`}
                >
                  <div className="item-label">
                    <span className="icon">{getCategoryIcon(catName)}</span>
                    <span>{catName}</span>
                  </div>
